...
model <- lm(charges ~ age + sex + bmi + smoker + region, data = train_data)
...
dum <- data.frame(age = 24,
                  ...
                  smoker = 1, 
                  region = 3)
print(paste0("Charges Prediction for dum: ", round(predict(model, dum), 2)))
```

### Decision Tree
```{r}
# build the decision tree model
model <- rpart(smoker ~ age + sex + bmi + region, data = train_data, 
               method = "class")
...
confusionMatrix(predictions, test_data$smoker)
